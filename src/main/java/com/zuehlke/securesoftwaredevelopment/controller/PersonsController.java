...
public class PersonsController {
    ...
    public String updatePerson(Person person, HttpSession session, @RequestParam("csrfToken") String csrfToken) throws
        ...
        if(person.getId().equals(Integer.toString(current.getId()))){
            personRepository.update(person);
            return "redirect:/myprofile/";
        }
            if(AuthorityUtils.authorityListToSet(SecurityContextHolder.getContext().getAuthentication().getAuthorities()).contains("UPDATE_PERSON")) {
                personRepository.update(person);
