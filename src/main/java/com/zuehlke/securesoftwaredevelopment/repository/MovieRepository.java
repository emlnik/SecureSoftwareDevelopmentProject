...
public class MovieRepository {
    ...
    public List<Movie> getAll() {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            LOG.warn("Error while getting movie list!");
        }
        return movieList;
    }
    ...
    public List<Movie> search(String searchTerm) throws SQLException {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            while (rs.next()) {
                ...
                LOG.warn("Error while searching the movie!");
            }
                    ...
                    Genre genre = genreList.stream().filter(g -> {
                        try {
                            return g.getId() == rs2.getInt(2);
                                ...
                                LOG.warn("Error while searching genre of the movie!");
    ...
    public long create(NewMovie movie, List<Genre> genresToInsert) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            String afterInsert=getMovies();
            if (generatedKeys.next()) {
                id = generatedKeys.getLong(1);
                long finalId = id;
                Entity ent= new Entity("Added new movie",finalId+"",beforeInsert,afterInsert);
                AuditLogger.getAuditLogger(MovieRepository.class).auditChange(ent);
                genresToInsert.stream().forEach(genre -> {
                    ...
                    try (PreparedStatement statement2 = connection.prepareStatement(query2);
                        ...
                        AuditLogger.getAuditLogger(MovieRepository.class).audit("Movie was successfully added with its genre");

                    } catch (SQLException e) {
                        LOG.warn("Error while inserting genre!");
                    }
                });
            }
        } catch (SQLException e) {
            LOG.warn("Error while inserting the movie");
        }
        return id;
    }

    public void delete(int movieId) {
        String beforeDelete = getMovies();
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            String afterDelete = getMovies();
            Entity ent= new Entity("Deleted movie",movieId+"",beforeDelete,afterDelete);
            AuditLogger.getAuditLogger(MovieRepository.class).auditChange(ent);

        } catch (SQLException e) {
            LOG.warn("Error while deleting the movie");
        }
    }
    ...
    private Movie createMovieFromResultSet(ResultSet rs) throws SQLException {
      ...
      //  LOG.warn("Creating the movie was successfully done!");
